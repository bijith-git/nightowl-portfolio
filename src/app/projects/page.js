export default function Project() {
  return (
    <div className="mil-content">
      <div id="swupMain" className="mil-main-transition">
        <section className="mil-portfolio-slider-frame">
          <div className="mil-animation-frame">
            <div
              className="mil-animation mil-position-4 mil-dark mil-scale"
              data-value-1={1}
              data-value-2={2}
              style={{ top: "-60px", right: "-4px" }}
            />
          </div>
          <div className="container">
            <div className="row align-items-end">
              <div className="col-lg-9">
                <div className="swiper-container mil-portfolio-slider mil-up">
                  <div className="swiper-wrapper">
                    <div className="swiper-slide">
                      <div
                        className="mil-portfolio-item mil-slider-item"
                        data-swiper-parallax={-30}
                      >
                        <div className="mil-cover-frame mil-drag">
                          <div
                            className="mil-cover"
                            data-swiper-parallax-scale="1.3"
                          >
                            <img src="img/works/1.jpg" alt="cover" />
                          </div>
                        </div>
                        <div
                          className="mil-descr"
                          data-swiper-parallax-x="104%"
                          data-swiper-parallax-opacity={0}
                        >
                          <div
                            className="mil-descr-text"
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <div className="mil-labels mil-mb-15">
                              <div className="mil-label mil-upper mil-accent">
                                TECHNOLOGY
                              </div>
                              <div className="mil-label mil-upper">
                                may 24 2023
                              </div>
                            </div>
                            <h5>Interior design studio</h5>
                          </div>
                          <div
                            data-swiper-parallax-y={120}
                            data-swiper-parallax-opacity={0}
                          >
                            <a
                              href="/projects"
                              className="mil-button mil-arrow-place"
                            >
                              <span>View Project</span>
                            </a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="swiper-slide">
                      <div
                        className="mil-portfolio-item mil-slider-item"
                        data-swiper-parallax={-30}
                      >
                        <div className="mil-cover-frame mil-drag">
                          <div
                            className="mil-cover"
                            data-swiper-parallax-scale="1.3"
                          >
                            <img src="img/works/2.jpg" alt="cover" />
                          </div>
                        </div>
                        <div
                          className="mil-descr"
                          data-swiper-parallax-x="104%"
                          data-swiper-parallax-opacity={0}
                        >
                          <div
                            className="mil-descr-text"
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <div className="mil-labels mil-mb-15">
                              <div className="mil-label mil-upper mil-accent">
                                TECHNOLOGY
                              </div>
                              <div className="mil-label mil-upper">
                                may 24 2023
                              </div>
                            </div>
                            <h5>Home Security Camera</h5>
                          </div>
                          <div
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <a
                              href="project-2.html"
                              className="mil-button mil-arrow-place"
                            >
                              <span>View Project</span>
                            </a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="swiper-slide">
                      <div
                        className="mil-portfolio-item mil-slider-item"
                        data-swiper-parallax={-30}
                      >
                        <div className="mil-cover-frame mil-drag">
                          <div
                            className="mil-cover"
                            data-swiper-parallax-scale="1.3"
                          >
                            <img src="img/works/3.jpg" alt="cover" />
                          </div>
                        </div>
                        <div
                          className="mil-descr"
                          data-swiper-parallax-x="104%"
                          data-swiper-parallax-opacity={0}
                        >
                          <div
                            className="mil-descr-text"
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <div className="mil-labels mil-mb-15">
                              <div className="mil-label mil-upper mil-accent">
                                TECHNOLOGY
                              </div>
                              <div className="mil-label mil-upper">
                                may 24 2023
                              </div>
                            </div>
                            <h5>Kemia Honest Skincare</h5>
                          </div>
                          <div
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <a
                              href="project-3.html"
                              className="mil-button mil-arrow-place"
                            >
                              <span>View Project</span>
                            </a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="swiper-slide">
                      <div
                        className="mil-portfolio-item mil-slider-item"
                        data-swiper-parallax={-30}
                      >
                        <div className="mil-cover-frame mil-drag">
                          <div
                            className="mil-cover"
                            data-swiper-parallax-scale="1.3"
                          >
                            <img src="img/works/4.jpg" alt="cover" />
                          </div>
                        </div>
                        <div
                          className="mil-descr"
                          data-swiper-parallax-x="104%"
                          data-swiper-parallax-opacity={0}
                        >
                          <div
                            className="mil-descr-text"
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <div className="mil-labels mil-mb-15">
                              <div className="mil-label mil-upper mil-accent">
                                TECHNOLOGY
                              </div>
                              <div className="mil-label mil-upper">
                                may 24 2023
                              </div>
                            </div>
                            <h5>Cascade of Lava</h5>
                          </div>
                          <div
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <a
                              href="project-4.html"
                              className="mil-button mil-arrow-place"
                            >
                              <span>View Project</span>
                            </a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="swiper-slide">
                      <div
                        className="mil-portfolio-item mil-slider-item"
                        data-swiper-parallax={-30}
                      >
                        <div className="mil-cover-frame mil-drag">
                          <div
                            className="mil-cover"
                            data-swiper-parallax-scale="1.3"
                          >
                            <img src="img/works/5.jpg" alt="cover" />
                          </div>
                        </div>
                        <div
                          className="mil-descr"
                          data-swiper-parallax-x="104%"
                          data-swiper-parallax-opacity={0}
                        >
                          <div
                            className="mil-descr-text"
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <div className="mil-labels mil-mb-15">
                              <div className="mil-label mil-upper mil-accent">
                                TECHNOLOGY
                              </div>
                              <div className="mil-label mil-upper">
                                may 24 2023
                              </div>
                            </div>
                            <h5>Air Pro by Molekule</h5>
                          </div>
                          <div
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <a
                              href="project-5.html"
                              className="mil-button mil-arrow-place"
                            >
                              <span>View Project</span>
                            </a>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="swiper-slide">
                      <div
                        className="mil-portfolio-item mil-slider-item"
                        data-swiper-parallax={-30}
                      >
                        <div className="mil-cover-frame mil-drag">
                          <div
                            className="mil-cover"
                            data-swiper-parallax-scale="1.3"
                          >
                            <img src="img/works/6.jpg" alt="cover" />
                          </div>
                        </div>
                        <div
                          className="mil-descr"
                          data-swiper-parallax-x="104%"
                          data-swiper-parallax-opacity={0}
                        >
                          <div
                            className="mil-descr-text"
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <div className="mil-labels mil-mb-15">
                              <div className="mil-label mil-upper mil-accent">
                                TECHNOLOGY
                              </div>
                              <div className="mil-label mil-upper">
                                may 24 2023
                              </div>
                            </div>
                            <h5>Tony&apos;s Chocolonely</h5>
                          </div>
                          <div
                            data-swiper-parallax-y="100%"
                            data-swiper-parallax-opacity={0}
                          >
                            <a
                              href="project-6.html"
                              className="mil-button mil-arrow-place"
                            >
                              <span>View Project</span>
                            </a>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-lg-3 mil-relative">
                <div className="mil-portfolio-nav">
                  <div className="mil-portfolio-btns-frame">
                    <div className="swiper-portfolio-pagination" />
                    <div className="mil-slider-nav">
                      <div className="mil-slider-arrow mil-prev mil-portfolio-prev mil-arrow-place" />
                      <div className="mil-slider-arrow mil-portfolio-next mil-arrow-place" />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
        <div className="mil-hidden-elements">
          <div className="mil-dodecahedron">
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
          </div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            className="mil-arrow"
          >
            <path d="M 14 5.3417969 C 13.744125 5.3417969 13.487969 5.4412187 13.292969 5.6367188 L 13.207031 5.7226562 C 12.816031 6.1136563 12.816031 6.7467188 13.207031 7.1367188 L 17.070312 11 L 4 11 C 3.448 11 3 11.448 3 12 C 3 12.552 3.448 13 4 13 L 17.070312 13 L 13.207031 16.863281 C 12.816031 17.254281 12.816031 17.887344 13.207031 18.277344 L 13.292969 18.363281 C 13.683969 18.754281 14.317031 18.754281 14.707031 18.363281 L 20.363281 12.707031 C 20.754281 12.316031 20.754281 11.682969 20.363281 11.292969 L 14.707031 5.6367188 C 14.511531 5.4412187 14.255875 5.3417969 14 5.3417969 z" />
          </svg>
          <svg
            width={250}
            viewBox="0 0 300 1404"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="mil-lines"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M1 892L1 941H299V892C299 809.71 232.29 743 150 743C67.7096 743 1 809.71 1 892ZM0 942H300V892C300 809.157 232.843 742 150 742C67.1573 742 0 809.157 0 892L0 942Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 146V97L1 97V146C1 228.29 67.7096 295 150 295C232.29 295 299 228.29 299 146ZM300 96L0 96V146C0 228.843 67.1573 296 150 296C232.843 296 300 228.843 300 146V96Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 1H1V1403H299V1ZM0 0V1404H300V0H0Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 -4.37115e-08L150 1404L149 1404L149 0L150 -4.37115e-08Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 1324C232.29 1324 299 1257.29 299 1175C299 1092.71 232.29 1026 150 1026C67.7096 1026 1 1092.71 1 1175C1 1257.29 67.7096 1324 150 1324ZM150 1325C232.843 1325 300 1257.84 300 1175C300 1092.16 232.843 1025 150 1025C67.1573 1025 0 1092.16 0 1175C0 1257.84 67.1573 1325 150 1325Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M300 1175H0V1174H300V1175Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 678C232.29 678 299 611.29 299 529C299 446.71 232.29 380 150 380C67.7096 380 1 446.71 1 529C1 611.29 67.7096 678 150 678ZM150 679C232.843 679 300 611.843 300 529C300 446.157 232.843 379 150 379C67.1573 379 0 446.157 0 529C0 611.843 67.1573 679 150 679Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 380H1V678H299V380ZM0 379V679H300V379H0Z"
              className="mil-move"
            />
          </svg>
        </div>
        {/* hidden elements end */}
      </div>
    </div>
  );
}

export function ProjectType1() {
  return (
    <div className="mil-content">
      <div id="swupMain" className="mil-main-transition">
        {/* banner */}
        <div className="mil-inner-banner">
          <div className="mil-banner-content mil-up">
            <div className="mil-animation-frame">
              <div
                className="mil-animation mil-position-4 mil-dark mil-scale"
                data-value-1={6}
                data-value-2="1.4"
              />
            </div>
            <div className="container">
              <ul className="mil-breadcrumbs mil-mb-60">
                <li>
                  <a href="/">Homepage</a>
                </li>
                <li>
                  <a href="/portfolio">Portfolio</a>
                </li>
                <li>
                  <a href="/project">Project</a>
                </li>
              </ul>
              <h1 className="mil-mb-60">
                Interior Design <span className="mil-thin">Studio</span>
              </h1>
              <a
                href="#project"
                className="mil-link mil-dark mil-arrow-place mil-down-arrow"
              >
                <span>Read more</span>
              </a>
            </div>
          </div>
        </div>
        <section className="mil-p-120-0">
          <div className="container mil-p-0-120" id="project">
            <div className="mil-image-frame mil-horizontal mil-up">
              <img src="img/works/1/1.jpg" alt="image" />
              <a
                data-fancybox="gallery"
                data-no-swup=""
                href="img/works/1/1.jpg"
                className="mil-zoom-btn"
              >
                <img src="img/icons/zoom.svg" alt="zoom" />
              </a>
            </div>
            <div className="mil-info mil-up">
              <div>
                Client: &nbsp;;<span className="mil-dark">Envato Market</span>
              </div>
              <div>
                Date: &nbsp;;<span className="mil-dark">April 2023</span>
              </div>
              <div>
                Author: &nbsp;;<span className="mil-dark">Paul Trueman</span>
              </div>
            </div>
            <div className="mil-p-120-0">
              <div className="row">
                <div className="col-lg-6">
                  <div className="mil-image-frame mil-square mil-up mil-mb-30">
                    <img src="img/works/1/2.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/1/2.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="col-lg-6">
                  <div className="mil-image-frame mil-square mil-up mil-mb-30">
                    <img src="img/works/1/3.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/1/3.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="col-lg-6">
                  <div className="mil-image-frame mil-square mil-up mil-mb-30">
                    <img src="img/works/1/4.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/1/4.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="col-lg-6">
                  <div className="mil-image-frame mil-square mil-up mil-mb-30">
                    <img src="img/works/1/5.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/1/5.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
              </div>
              <div className="row justify-content-between mil-p-90-120">
                <div className="col-lg-5">
                  <h3 className="mil-up mil-mb-60">
                    Simplicity, elegance, innovation!
                  </h3>
                </div>
                <div className="col-lg-6">
                  <p className="mil-up mil-mb-30">
                    A home surveillance camera that pays great attention to
                    security and user privacy, featuring two modes to provide
                    security while protecting personal privacy.The camera has an
                    open and closed mode, we define the product to have clear
                    two sides, expressing two working states and emotions.
                  </p>
                  <p className="mil-up">
                    Presents a simple and quiet state when not in use,
                    delivering a gentle and security.At the same time, the
                    camera can adapt to a variety of environments, providing
                    elegant ways of wall hanging and standing installation.
                  </p>
                </div>
              </div>
              <div className="row mil-p-0-90">
                <div className="col-lg-6">
                  <div className="mil-image-frame mil-vertical mil-up mil-mb-30">
                    <img src="img/works/1/6.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/1/6.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="col-lg-6">
                  <div className="mil-image-frame mil-vertical mil-up mil-mb-30">
                    <img src="img/works/1/7.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/1/7.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
              </div>
            </div>
            <div className="mil-works-nav mil-up">
              <a
                href="#."
                className="mil-link mil-dark mil-arrow-place mil-icon-left mil-disabled"
              >
                <span>Prev project</span>
              </a>
              <a href="/portfolio" className="mil-link mil-dark">
                <span>All projects</span>
              </a>
              <a
                href="project-2.html"
                className="mil-link mil-dark mil-arrow-place"
              >
                <span>Next project</span>
              </a>
            </div>
          </div>
        </section>
        {/* project end */}
        {/* call to action */}
        <section className="mil-soft-bg">
          <div className="container mil-p-120-120">
            <div className="row">
              <div className="col-lg-10">
                <span className="mil-suptitle mil-suptitle-right mil-suptitle-dark mil-up">
                  Looking to make your mark? We&apos;ll help you turn <br />
                  your project into a success story.
                </span>
              </div>
            </div>
            <div className="mil-center">
              <h2 className="mil-up mil-mb-60">
                Ready to bring your
                <span className="mil-thin">ideas to</span> life? <br />
                We&apos;re <span className="mil-thin">here to help</span>
              </h2>
              <div className="mil-up">
                <a href="contact.html" className="mil-button mil-arrow-place">
                  <span>Contact us</span>
                </a>
              </div>
            </div>
          </div>
        </section>

        <div className="mil-hidden-elements">
          <div className="mil-dodecahedron">
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
          </div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            className="mil-arrow"
          >
            <path d="M 14 5.3417969 C 13.744125 5.3417969 13.487969 5.4412187 13.292969 5.6367188 L 13.207031 5.7226562 C 12.816031 6.1136563 12.816031 6.7467188 13.207031 7.1367188 L 17.070312 11 L 4 11 C 3.448 11 3 11.448 3 12 C 3 12.552 3.448 13 4 13 L 17.070312 13 L 13.207031 16.863281 C 12.816031 17.254281 12.816031 17.887344 13.207031 18.277344 L 13.292969 18.363281 C 13.683969 18.754281 14.317031 18.754281 14.707031 18.363281 L 20.363281 12.707031 C 20.754281 12.316031 20.754281 11.682969 20.363281 11.292969 L 14.707031 5.6367188 C 14.511531 5.4412187 14.255875 5.3417969 14 5.3417969 z" />
          </svg>
          <svg
            width={250}
            viewBox="0 0 300 1404"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="mil-lines"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M1 892L1 941H299V892C299 809.71 232.29 743 150 743C67.7096 743 1 809.71 1 892ZM0 942H300V892C300 809.157 232.843 742 150 742C67.1573 742 0 809.157 0 892L0 942Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 146V97L1 97V146C1 228.29 67.7096 295 150 295C232.29 295 299 228.29 299 146ZM300 96L0 96V146C0 228.843 67.1573 296 150 296C232.843 296 300 228.843 300 146V96Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 1H1V1403H299V1ZM0 0V1404H300V0H0Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 -4.37115e-08L150 1404L149 1404L149 0L150 -4.37115e-08Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 1324C232.29 1324 299 1257.29 299 1175C299 1092.71 232.29 1026 150 1026C67.7096 1026 1 1092.71 1 1175C1 1257.29 67.7096 1324 150 1324ZM150 1325C232.843 1325 300 1257.84 300 1175C300 1092.16 232.843 1025 150 1025C67.1573 1025 0 1092.16 0 1175C0 1257.84 67.1573 1325 150 1325Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M300 1175H0V1174H300V1175Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 678C232.29 678 299 611.29 299 529C299 446.71 232.29 380 150 380C67.7096 380 1 446.71 1 529C1 611.29 67.7096 678 150 678ZM150 679C232.843 679 300 611.843 300 529C300 446.157 232.843 379 150 379C67.1573 379 0 446.157 0 529C0 611.843 67.1573 679 150 679Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 380H1V678H299V380ZM0 379V679H300V379H0Z"
              className="mil-move"
            />
          </svg>
        </div>
        {/* hidden elements end */}
      </div>
    </div>
  );
}
export function ProjectType2() {
  return (
    <div className="mil-content">
      <div id="swupMain" className="mil-main-transition">
        {/* banner */}
        <div className="mil-inner-banner">
          <div className="mil-banner-content mil-up">
            <div className="mil-animation-frame">
              <div
                className="mil-animation mil-position-4 mil-dark mil-scale"
                data-value-1={6}
                data-value-2="1.4"
              />
            </div>
            <div className="container">
              <ul className="mil-breadcrumbs mil-mb-60">
                <li>
                  <a href="/">Homepage</a>
                </li>
                <li>
                  <a href="/portfolio">Portfolio</a>
                </li>
                <li>
                  <a href="/projects">Project</a>
                </li>
              </ul>
              <h1 className="mil-mb-60">
                Interior Design <span className="mil-thin">Studio</span>
              </h1>
              <a
                href="#project"
                className="mil-link mil-dark mil-arrow-place mil-down-arrow"
              >
                <span>Read more</span>
              </a>
            </div>
          </div>
        </div>
        {/* banner end */}
        {/* project */}
        <section>
          <div className="container mil-p-120-120" id="project">
            <div className="row justify-content-between mil-mb-120">
              <div className="col-lg-4">
                <div className="mil-p-0-120">
                  <ul className="mil-service-list mil-dark mil-mb-60">
                    <li className="mil-up">
                      Client: &nbsp;
                      <span className="mil-dark">Envato Market</span>
                    </li>
                    <li className="mil-up">
                      Date: &nbsp;<span className="mil-dark">April 2023</span>
                    </li>
                    <li className="mil-up">
                      Author: &nbsp;
                      <span className="mil-dark">Paul Trueman</span>
                    </li>
                  </ul>
                  <h5 className="mil-up mil-mb-30">
                    Simplicity, elegance, innovation!
                  </h5>
                  <p className="mil-up mil-mb-30">
                    A home surveillance camera that pays great attention to
                    security and user privacy, featuring two modes to provide
                    security while protecting personal privacy.The camera has an
                    open and closed mode, we define the product to have clear
                    two sides, expressing two working states and emotions.
                  </p>
                  <p className="mil-up mil-mb-60">
                    Presents a simple and quiet state when not in use,
                    delivering a gentle and security.At the same time, the
                    camera can adapt to a variety of environments, providing
                    elegant ways of wall hanging and standing installation.
                  </p>
                  <a
                    data-no-swup=""
                    href="https://www.behance.net/gallery/117994149/Home-Security-Camera?tracking_source=curated_galleries_product-design"
                    target="_blank"
                    className="mil-link mil-dark mil-up mil-arrow-place"
                  >
                    <span>Visit website</span>
                  </a>
                </div>
              </div>
              <div className="col-lg-7">
                <div className="mil-image-frame mil-horizontal mil-up mil-mb-30">
                  <img src="img/works/2/1.gif" alt="image" />
                  <a
                    data-fancybox="gallery"
                    data-no-swup=""
                    href="img/works/2/1.gif"
                    className="mil-zoom-btn"
                  >
                    <img src="img/icons/zoom.svg" alt="zoom" />
                  </a>
                </div>
                <div className="mil-image-frame mil-horizontal mil-up mil-mb-30">
                  <img src="img/works/2/2.jpg" alt="image" />
                  <a
                    data-fancybox="gallery"
                    data-no-swup=""
                    href="img/works/2/2.jpg"
                    className="mil-zoom-btn"
                  >
                    <img src="img/icons/zoom.svg" alt="zoom" />
                  </a>
                </div>
                <div className="mil-image-frame mil-horizontal mil-up mil-mb-30">
                  <img src="img/works/2/3.jpg" alt="image" />
                  <a
                    data-fancybox="gallery"
                    data-no-swup=""
                    href="img/works/2/3.jpg"
                    className="mil-zoom-btn"
                  >
                    <img src="img/icons/zoom.svg" alt="zoom" />
                  </a>
                </div>
                <div className="mil-image-frame mil-horizontal mil-up">
                  <img src="img/works/2/4.jpg" alt="image" />
                  <a
                    data-fancybox="gallery"
                    data-no-swup=""
                    href="img/works/2/4.jpg"
                    className="mil-zoom-btn"
                  >
                    <img src="img/icons/zoom.svg" alt="zoom" />
                  </a>
                </div>
              </div>
            </div>
            <div className="mil-works-nav mil-up">
              <a
                href="/projects"
                className="mil-link mil-dark mil-arrow-place mil-icon-left"
              >
                <span>Prev project</span>
              </a>
              <a href="/portfolio" className="mil-link mil-dark">
                <span>All projects</span>
              </a>
              <a
                href="project-3.html"
                className="mil-link mil-dark mil-arrow-place"
              >
                <span>Next project</span>
              </a>
            </div>
          </div>
        </section>
        {/* project end */}
        {/* call to action */}
        <section className="mil-soft-bg">
          <div className="container mil-p-120-120">
            <div className="row">
              <div className="col-lg-10">
                <span className="mil-suptitle mil-suptitle-right mil-suptitle-dark mil-up">
                  Looking to make your mark? We&apos;ll help you turn <br />
                  your project into a success story.
                </span>
              </div>
            </div>
            <div className="mil-center">
              <h2 className="mil-up mil-mb-60">
                Ready to bring your
                <span className="mil-thin">ideas to</span> life? <br />
                We&apos;re <span className="mil-thin">here to help</span>
              </h2>
              <div className="mil-up">
                <a href="contact.html" className="mil-button mil-arrow-place">
                  <span>Contact us</span>
                </a>
              </div>
            </div>
          </div>
        </section>
        {/* call to action end */}
        {/* hidden elements */}
        <div className="mil-hidden-elements">
          <div className="mil-dodecahedron">
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
          </div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            className="mil-arrow"
          >
            <path d="M 14 5.3417969 C 13.744125 5.3417969 13.487969 5.4412187 13.292969 5.6367188 L 13.207031 5.7226562 C 12.816031 6.1136563 12.816031 6.7467188 13.207031 7.1367188 L 17.070312 11 L 4 11 C 3.448 11 3 11.448 3 12 C 3 12.552 3.448 13 4 13 L 17.070312 13 L 13.207031 16.863281 C 12.816031 17.254281 12.816031 17.887344 13.207031 18.277344 L 13.292969 18.363281 C 13.683969 18.754281 14.317031 18.754281 14.707031 18.363281 L 20.363281 12.707031 C 20.754281 12.316031 20.754281 11.682969 20.363281 11.292969 L 14.707031 5.6367188 C 14.511531 5.4412187 14.255875 5.3417969 14 5.3417969 z" />
          </svg>
          <svg
            width={250}
            viewBox="0 0 300 1404"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="mil-lines"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M1 892L1 941H299V892C299 809.71 232.29 743 150 743C67.7096 743 1 809.71 1 892ZM0 942H300V892C300 809.157 232.843 742 150 742C67.1573 742 0 809.157 0 892L0 942Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 146V97L1 97V146C1 228.29 67.7096 295 150 295C232.29 295 299 228.29 299 146ZM300 96L0 96V146C0 228.843 67.1573 296 150 296C232.843 296 300 228.843 300 146V96Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 1H1V1403H299V1ZM0 0V1404H300V0H0Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 -4.37115e-08L150 1404L149 1404L149 0L150 -4.37115e-08Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 1324C232.29 1324 299 1257.29 299 1175C299 1092.71 232.29 1026 150 1026C67.7096 1026 1 1092.71 1 1175C1 1257.29 67.7096 1324 150 1324ZM150 1325C232.843 1325 300 1257.84 300 1175C300 1092.16 232.843 1025 150 1025C67.1573 1025 0 1092.16 0 1175C0 1257.84 67.1573 1325 150 1325Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M300 1175H0V1174H300V1175Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 678C232.29 678 299 611.29 299 529C299 446.71 232.29 380 150 380C67.7096 380 1 446.71 1 529C1 611.29 67.7096 678 150 678ZM150 679C232.843 679 300 611.843 300 529C300 446.157 232.843 379 150 379C67.1573 379 0 446.157 0 529C0 611.843 67.1573 679 150 679Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 380H1V678H299V380ZM0 379V679H300V379H0Z"
              className="mil-move"
            />
          </svg>
        </div>
        {/* hidden elements end */}
      </div>
    </div>
  );
}
export function ProjectType3() {
  return (
    <div className="mil-content">
      <div id="swupMain" className="mil-main-transition">
        {/* banner */}
        <div className="mil-inner-banner">
          <div className="mil-banner-content mil-up">
            <div className="mil-animation-frame">
              <div
                className="mil-animation mil-position-4 mil-dark mil-scale"
                data-value-1={6}
                data-value-2="1.4"
              />
            </div>
            <div className="container">
              <ul className="mil-breadcrumbs mil-mb-60">
                <li>
                  <a href="/">Homepage</a>
                </li>
                <li>
                  <a href="/portfolio">Portfolio</a>
                </li>
                <li>
                  <a href="/projects">Project</a>
                </li>
              </ul>
              <h1 className="mil-mb-60">
                Kemia Honest <span className="mil-thin">Skincare</span>
              </h1>
              <a
                href="#project"
                className="mil-link mil-dark mil-arrow-place mil-down-arrow"
              >
                <span>Read more</span>
              </a>
            </div>
          </div>
        </div>
        {/* banner end */}
        {/* project */}
        <section id="project">
          <div className="container mil-p-120-120">
            <div className="swiper-container mil-1-slider mil-up">
              <div className="swiper-wrapper">
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-horizontal mil-drag">
                    <img src="img/works/3/1.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/3/1.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-horizontal mil-drag">
                    <img src="img/works/3/2.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/3/2.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-horizontal mil-drag">
                    <img src="img/works/3/3.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/3/3.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-horizontal mil-drag">
                    <img src="img/works/3/4.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/3/4.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
              </div>
            </div>
            <div className="mil-info mil-up">
              <div>
                Client: &nbsp;<span className="mil-dark">Envato Market</span>
              </div>
              <div>
                Date: &nbsp;<span className="mil-dark">April 2023</span>
              </div>
              <div>
                Author: &nbsp;<span className="mil-dark">Paul Trueman</span>
              </div>
            </div>
            <div className="row justify-content-between mil-p-120-90">
              <div className="col-lg-5">
                <h3 className="mil-up mil-mb-60">
                  Simplicity, elegance, innovation!
                </h3>
              </div>
              <div className="col-lg-6">
                <p className="mil-up mil-mb-30">
                  A home surveillance camera that pays great attention to
                  security and user privacy, featuring two modes to provide
                  security while protecting personal privacy.The camera has an
                  open and closed mode, we define the product to have clear two
                  sides, expressing two working states and emotions.
                </p>
                <p className="mil-up mil-mb-30">
                  Presents a simple and quiet state when not in use, delivering
                  a gentle and security.At the same time, the camera can adapt
                  to a variety of environments, providing elegant ways of wall
                  hanging and standing installation.
                </p>
              </div>
            </div>
            <div className="mil-works-nav mil-up">
              <a
                href="project-2.html"
                className="mil-link mil-dark mil-arrow-place mil-icon-left"
              >
                <span>Prev project</span>
              </a>
              <a href="/portfolio" className="mil-link mil-dark">
                <span>All projects</span>
              </a>
              <a
                href="project-4.html"
                className="mil-link mil-dark mil-arrow-place"
              >
                <span>Next project</span>
              </a>
            </div>
          </div>
        </section>
        {/* project end */}
        {/* call to action */}
        <section className="mil-soft-bg">
          <div className="container mil-p-120-120">
            <div className="row">
              <div className="col-lg-10">
                <span className="mil-suptitle mil-suptitle-right mil-suptitle-dark mil-up">
                  Looking to make your mark? We&apos;ll help you turn <br />
                  your project into a success story.
                </span>
              </div>
            </div>
            <div className="mil-center">
              <h2 className="mil-up mil-mb-60">
                Ready to bring your
                <span className="mil-thin">ideas to</span> life? <br />
                We&apos;re <span className="mil-thin">here to help</span>
              </h2>
              <div className="mil-up">
                <a href="contact.html" className="mil-button mil-arrow-place">
                  <span>Contact us</span>
                </a>
              </div>
            </div>
          </div>
        </section>
        <div className="mil-hidden-elements">
          <div className="mil-dodecahedron">
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
          </div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            className="mil-arrow"
          >
            <path d="M 14 5.3417969 C 13.744125 5.3417969 13.487969 5.4412187 13.292969 5.6367188 L 13.207031 5.7226562 C 12.816031 6.1136563 12.816031 6.7467188 13.207031 7.1367188 L 17.070312 11 L 4 11 C 3.448 11 3 11.448 3 12 C 3 12.552 3.448 13 4 13 L 17.070312 13 L 13.207031 16.863281 C 12.816031 17.254281 12.816031 17.887344 13.207031 18.277344 L 13.292969 18.363281 C 13.683969 18.754281 14.317031 18.754281 14.707031 18.363281 L 20.363281 12.707031 C 20.754281 12.316031 20.754281 11.682969 20.363281 11.292969 L 14.707031 5.6367188 C 14.511531 5.4412187 14.255875 5.3417969 14 5.3417969 z" />
          </svg>
          <svg
            width={250}
            viewBox="0 0 300 1404"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="mil-lines"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M1 892L1 941H299V892C299 809.71 232.29 743 150 743C67.7096 743 1 809.71 1 892ZM0 942H300V892C300 809.157 232.843 742 150 742C67.1573 742 0 809.157 0 892L0 942Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 146V97L1 97V146C1 228.29 67.7096 295 150 295C232.29 295 299 228.29 299 146ZM300 96L0 96V146C0 228.843 67.1573 296 150 296C232.843 296 300 228.843 300 146V96Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 1H1V1403H299V1ZM0 0V1404H300V0H0Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 -4.37115e-08L150 1404L149 1404L149 0L150 -4.37115e-08Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 1324C232.29 1324 299 1257.29 299 1175C299 1092.71 232.29 1026 150 1026C67.7096 1026 1 1092.71 1 1175C1 1257.29 67.7096 1324 150 1324ZM150 1325C232.843 1325 300 1257.84 300 1175C300 1092.16 232.843 1025 150 1025C67.1573 1025 0 1092.16 0 1175C0 1257.84 67.1573 1325 150 1325Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M300 1175H0V1174H300V1175Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 678C232.29 678 299 611.29 299 529C299 446.71 232.29 380 150 380C67.7096 380 1 446.71 1 529C1 611.29 67.7096 678 150 678ZM150 679C232.843 679 300 611.843 300 529C300 446.157 232.843 379 150 379C67.1573 379 0 446.157 0 529C0 611.843 67.1573 679 150 679Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 380H1V678H299V380ZM0 379V679H300V379H0Z"
              className="mil-move"
            />
          </svg>
        </div>
        {/* hidden elements end */}
      </div>
    </div>
  );
}
export function ProjectType4() {
  return (
    <div className="mil-content">
      <div id="swupMain" className="mil-main-transition">
        {/* banner */}
        <div className="mil-inner-banner">
          <div className="mil-banner-content mil-up">
            <div className="mil-animation-frame">
              <div
                className="mil-animation mil-position-4 mil-dark mil-scale"
                data-value-1={6}
                data-value-2="1.4"
              />
            </div>
            <div className="container">
              <ul className="mil-breadcrumbs mil-mb-60">
                <li>
                  <a href="/">Homepage</a>
                </li>
                <li>
                  <a href="/portfolio">Portfolio</a>
                </li>
                <li>
                  <a href="/projects">Project</a>
                </li>
              </ul>
              <h1 className="mil-mb-60">
                Cascade <span className="mil-thin">of Lava</span>
              </h1>
              <a
                href="#project"
                className="mil-link mil-dark mil-arrow-place mil-down-arrow"
              >
                <span>Read more</span>
              </a>
            </div>
          </div>
        </div>
        {/* banner end */}
        {/* project */}
        <section id="project">
          <div className="container mil-p-120-120">
            <div className="swiper-container mil-2-slider mil-up">
              <div className="swiper-wrapper">
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-vertical mil-drag">
                    <img src="img/works/4/1.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/4/1.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-vertical mil-drag">
                    <img src="img/works/4/2.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/4/2.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-vertical mil-drag">
                    <img src="img/works/4/3.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/4/3.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-vertical mil-drag">
                    <img src="img/works/4/4.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/4/4.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-vertical mil-drag">
                    <img src="img/works/4/5.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/4/5.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-vertical mil-drag">
                    <img src="img/works/4/6.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/4/6.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
              </div>
            </div>
            <div className="mil-info mil-up">
              <div>
                Client: &nbsp;<span className="mil-dark">Envato Market</span>
              </div>
              <div>
                Date: &nbsp;<span className="mil-dark">April 2023</span>
              </div>
              <div>
                Author: &nbsp;<span className="mil-dark">Paul Trueman</span>
              </div>
            </div>
            <div className="row justify-content-center mil-center mil-p-120-120">
              <div className="col-lg-8">
                <h3 className="mil-up mil-mb-60">
                  Lava Photos taken at Hawaii <br />
                  Volcanoes National Park.
                </h3>
                <p className="mil-text-lg mil-up">
                  This collection of Hawaii photography is focused on the
                  immense power of lava at the Volcanos National Park on the Big
                  Island of Hawaii. One of the most truly mystical scenes I have
                  ever had the pleasure of capturing. For this collection I
                  wanted to showcase this beauty in my own artistic way, through
                  fine art landscape photography. However still keep these
                  scenes natural and true to what I witnessed on my journey. For
                  that reason, these images have been only lightly processed.
                  All of these images are single exposures, unless listed
                  otherwise in the caption below the photo.
                </p>
              </div>
            </div>
            <div className="mil-image-frame mil-horizontal mil-up mil-mb-120">
              <img src="img/works/4/7.jpg" alt="image" />
              <a
                data-fancybox="gallery"
                data-no-swup=""
                href="img/works/4/7.jpg"
                className="mil-zoom-btn"
              >
                <img src="img/icons/zoom.svg" alt="zoom" />
              </a>
            </div>
            <div className="mil-works-nav mil-up">
              <a
                href="project-3.html"
                className="mil-link mil-dark mil-arrow-place mil-icon-left"
              >
                <span>Prev project</span>
              </a>
              <a href="/portfolio" className="mil-link mil-dark">
                <span>All projects</span>
              </a>
              <a
                href="project-5.html"
                className="mil-link mil-dark mil-arrow-place"
              >
                <span>Next project</span>
              </a>
            </div>
          </div>
        </section>
        {/* project end */}
        {/* call to action */}
        <section className="mil-soft-bg">
          <div className="container mil-p-120-120">
            <div className="row">
              <div className="col-lg-10">
                <span className="mil-suptitle mil-suptitle-right mil-suptitle-dark mil-up">
                  Looking to make your mark? We&apos;ll help you turn <br />
                  your project into a success story.
                </span>
              </div>
            </div>
            <div className="mil-center">
              <h2 className="mil-up mil-mb-60">
                Ready to bring your
                <span className="mil-thin">ideas to</span> life? <br />
                We&apos;re <span className="mil-thin">here to help</span>
              </h2>
              <div className="mil-up">
                <a href="contact.html" className="mil-button mil-arrow-place">
                  <span>Contact us</span>
                </a>
              </div>
            </div>
          </div>
        </section>
        <div className="mil-hidden-elements">
          <div className="mil-dodecahedron">
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
          </div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            className="mil-arrow"
          >
            <path d="M 14 5.3417969 C 13.744125 5.3417969 13.487969 5.4412187 13.292969 5.6367188 L 13.207031 5.7226562 C 12.816031 6.1136563 12.816031 6.7467188 13.207031 7.1367188 L 17.070312 11 L 4 11 C 3.448 11 3 11.448 3 12 C 3 12.552 3.448 13 4 13 L 17.070312 13 L 13.207031 16.863281 C 12.816031 17.254281 12.816031 17.887344 13.207031 18.277344 L 13.292969 18.363281 C 13.683969 18.754281 14.317031 18.754281 14.707031 18.363281 L 20.363281 12.707031 C 20.754281 12.316031 20.754281 11.682969 20.363281 11.292969 L 14.707031 5.6367188 C 14.511531 5.4412187 14.255875 5.3417969 14 5.3417969 z" />
          </svg>
          <svg
            width={250}
            viewBox="0 0 300 1404"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="mil-lines"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M1 892L1 941H299V892C299 809.71 232.29 743 150 743C67.7096 743 1 809.71 1 892ZM0 942H300V892C300 809.157 232.843 742 150 742C67.1573 742 0 809.157 0 892L0 942Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 146V97L1 97V146C1 228.29 67.7096 295 150 295C232.29 295 299 228.29 299 146ZM300 96L0 96V146C0 228.843 67.1573 296 150 296C232.843 296 300 228.843 300 146V96Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 1H1V1403H299V1ZM0 0V1404H300V0H0Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 -4.37115e-08L150 1404L149 1404L149 0L150 -4.37115e-08Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 1324C232.29 1324 299 1257.29 299 1175C299 1092.71 232.29 1026 150 1026C67.7096 1026 1 1092.71 1 1175C1 1257.29 67.7096 1324 150 1324ZM150 1325C232.843 1325 300 1257.84 300 1175C300 1092.16 232.843 1025 150 1025C67.1573 1025 0 1092.16 0 1175C0 1257.84 67.1573 1325 150 1325Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M300 1175H0V1174H300V1175Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 678C232.29 678 299 611.29 299 529C299 446.71 232.29 380 150 380C67.7096 380 1 446.71 1 529C1 611.29 67.7096 678 150 678ZM150 679C232.843 679 300 611.843 300 529C300 446.157 232.843 379 150 379C67.1573 379 0 446.157 0 529C0 611.843 67.1573 679 150 679Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 380H1V678H299V380ZM0 379V679H300V379H0Z"
              className="mil-move"
            />
          </svg>
        </div>
        {/* hidden elements end */}
      </div>
    </div>
  );
}
export function ProjectType5() {
  return (
    <div className="mil-content">
      <div id="swupMain" className="mil-main-transition">
        {/* banner */}
        <div className="mil-inner-banner">
          <div className="mil-banner-content mil-up">
            <div className="mil-animation-frame">
              <div
                className="mil-animation mil-position-4 mil-dark mil-scale"
                data-value-1={6}
                data-value-2="1.4"
              />
            </div>
            <div className="container">
              <ul className="mil-breadcrumbs mil-mb-60">
                <li>
                  <a href="/">Homepage</a>
                </li>
                <li>
                  <a href="/portfolio">Portfolio</a>
                </li>
                <li>
                  <a href="/projects">Project</a>
                </li>
              </ul>
              <h1 className="mil-mb-60">
                Air Pro <span className="mil-thin">by Molekule</span>
              </h1>
              <a
                href="#project"
                className="mil-link mil-dark mil-arrow-place mil-down-arrow"
              >
                <span>Read more</span>
              </a>
            </div>
          </div>
        </div>
        {/* banner end */}
        {/* project */}
        <section className="mil-p-120-0">
          <div className="mil-image-frame mil-fw mil-up">
            <img src="img/works/5/1.jpg" alt="image" />
            <a
              data-fancybox="gallery"
              data-no-swup=""
              href="img/works/5/1.jpg"
              className="mil-zoom-btn"
            >
              <img src="img/icons/zoom.svg" alt="zoom" />
            </a>
          </div>
          <div className="container mil-p-0-120" id="project">
            <div className="mil-info mil-up">
              <div>
                Client: &nbsp;<span className="mil-dark">Envato Market</span>
              </div>
              <div>
                Date: &nbsp;<span className="mil-dark">April 2023</span>
              </div>
              <div>
                Author: &nbsp;<span className="mil-dark">Paul Trueman</span>
              </div>
            </div>
            <div className="mil-p-120-0">
              <div className="swiper-container mil-2-slider mil-up">
                <div className="swiper-wrapper">
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-square mil-drag">
                      <img src="img/works/5/2.jpg" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/5/2.jpg"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-square mil-drag">
                      <img src="img/works/5/3.gif" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/5/3.gif"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-square mil-drag">
                      <img src="img/works/5/4.jpg" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/5/4.jpg"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-square mil-drag">
                      <img src="img/works/5/5.gif" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/5/5.gif"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-square mil-drag">
                      <img src="img/works/5/6.jpg" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/5/6.jpg"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="row justify-content-between mil-p-120-120">
                <div className="col-lg-5">
                  <h3 className="mil-up mil-mb-60">
                    Simplicity, elegance, innovation!
                  </h3>
                </div>
                <div className="col-lg-6">
                  <p className="mil-up mil-mb-30">
                    A home surveillance camera that pays great attention to
                    security and user privacy, featuring two modes to provide
                    security while protecting personal privacy.The camera has an
                    open and closed mode, we define the product to have clear
                    two sides, expressing two working states and emotions.
                  </p>
                  <p className="mil-up">
                    Presents a simple and quiet state when not in use,
                    delivering a gentle and security.At the same time, the
                    camera can adapt to a variety of environments, providing
                    elegant ways of wall hanging and standing installation.
                  </p>
                </div>
              </div>
            </div>
            <div className="mil-image-frame mil-horizontal mil-up mil-mb-120">
              <img src="img/works/5/7.gif" alt="image" />
              <a
                data-fancybox="gallery"
                data-no-swup=""
                href="img/works/5/7.gif"
                className="mil-zoom-btn"
              >
                <img src="img/icons/zoom.svg" alt="zoom" />
              </a>
            </div>
            <div className="mil-works-nav mil-up">
              <a
                href="project-4.html"
                className="mil-link mil-dark mil-arrow-place mil-icon-left"
              >
                <span>Prev project</span>
              </a>
              <a href="/portfolio" className="mil-link mil-dark">
                <span>All projects</span>
              </a>
              <a
                href="project-6.html"
                className="mil-link mil-dark mil-arrow-place"
              >
                <span>Next project</span>
              </a>
            </div>
          </div>
        </section>
        {/* project end */}
        {/* call to action */}
        <section className="mil-soft-bg">
          <div className="container mil-p-120-120">
            <div className="row">
              <div className="col-lg-10">
                <span className="mil-suptitle mil-suptitle-right mil-suptitle-dark mil-up">
                  Looking to make your mark? We&apos;ll help you turn <br />
                  your project into a success story.
                </span>
              </div>
            </div>
            <div className="mil-center">
              <h2 className="mil-up mil-mb-60">
                Ready to bring your
                <span className="mil-thin">ideas to</span> life? <br />
                We&apos;re <span className="mil-thin">here to help</span>
              </h2>
              <div className="mil-up">
                <a href="contact.html" className="mil-button mil-arrow-place">
                  <span>Contact us</span>
                </a>
              </div>
            </div>
          </div>
        </section>
        <div className="mil-hidden-elements">
          <div className="mil-dodecahedron">
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
          </div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            className="mil-arrow"
          >
            <path d="M 14 5.3417969 C 13.744125 5.3417969 13.487969 5.4412187 13.292969 5.6367188 L 13.207031 5.7226562 C 12.816031 6.1136563 12.816031 6.7467188 13.207031 7.1367188 L 17.070312 11 L 4 11 C 3.448 11 3 11.448 3 12 C 3 12.552 3.448 13 4 13 L 17.070312 13 L 13.207031 16.863281 C 12.816031 17.254281 12.816031 17.887344 13.207031 18.277344 L 13.292969 18.363281 C 13.683969 18.754281 14.317031 18.754281 14.707031 18.363281 L 20.363281 12.707031 C 20.754281 12.316031 20.754281 11.682969 20.363281 11.292969 L 14.707031 5.6367188 C 14.511531 5.4412187 14.255875 5.3417969 14 5.3417969 z" />
          </svg>
          <svg
            width={250}
            viewBox="0 0 300 1404"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="mil-lines"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M1 892L1 941H299V892C299 809.71 232.29 743 150 743C67.7096 743 1 809.71 1 892ZM0 942H300V892C300 809.157 232.843 742 150 742C67.1573 742 0 809.157 0 892L0 942Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 146V97L1 97V146C1 228.29 67.7096 295 150 295C232.29 295 299 228.29 299 146ZM300 96L0 96V146C0 228.843 67.1573 296 150 296C232.843 296 300 228.843 300 146V96Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 1H1V1403H299V1ZM0 0V1404H300V0H0Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 -4.37115e-08L150 1404L149 1404L149 0L150 -4.37115e-08Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 1324C232.29 1324 299 1257.29 299 1175C299 1092.71 232.29 1026 150 1026C67.7096 1026 1 1092.71 1 1175C1 1257.29 67.7096 1324 150 1324ZM150 1325C232.843 1325 300 1257.84 300 1175C300 1092.16 232.843 1025 150 1025C67.1573 1025 0 1092.16 0 1175C0 1257.84 67.1573 1325 150 1325Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M300 1175H0V1174H300V1175Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 678C232.29 678 299 611.29 299 529C299 446.71 232.29 380 150 380C67.7096 380 1 446.71 1 529C1 611.29 67.7096 678 150 678ZM150 679C232.843 679 300 611.843 300 529C300 446.157 232.843 379 150 379C67.1573 379 0 446.157 0 529C0 611.843 67.1573 679 150 679Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 380H1V678H299V380ZM0 379V679H300V379H0Z"
              className="mil-move"
            />
          </svg>
        </div>
        {/* hidden elements end */}
      </div>
    </div>
  );
}
export function ProjectType6() {
  return (
    <div className="mil-content">
      <div id="swupMain" className="mil-main-transition">
        {/* banner */}
        <div className="mil-inner-banner">
          <div className="mil-banner-content mil-up">
            <div className="mil-animation-frame">
              <div
                className="mil-animation mil-position-4 mil-dark mil-scale"
                data-value-1={6}
                data-value-2="1.4"
              />
            </div>
            <div className="container">
              <ul className="mil-breadcrumbs mil-mb-60">
                <li>
                  <a href="/">Homepage</a>
                </li>
                <li>
                  <a href="/portfolio">Portfolio</a>
                </li>
                <li>
                  <a href="/projects">Project</a>
                </li>
              </ul>
              <h1 className="mil-mb-60">
                Air Pro <span className="mil-thin">by Molekule</span>
              </h1>
              <a
                href="#project"
                className="mil-link mil-dark mil-arrow-place mil-down-arrow"
              >
                <span>Read more</span>
              </a>
            </div>
          </div>
        </div>
        {/* banner end */}
        {/* project */}
        <section className="mil-p-120-120">
          <div className="mil-image-frame mil-fw mil-up">
            <img src="img/works/6/1.jpg" alt="image" />
            <a
              data-fancybox="gallery"
              data-no-swup=""
              href="img/works/6/1.jpg"
              className="mil-zoom-btn"
            >
              <img src="img/icons/zoom.svg" alt="zoom" />
            </a>
          </div>
          <div className="container" id="project">
            <div className="mil-info mil-up">
              <div>
                Client: &nbsp;<span className="mil-dark">Envato Market</span>
              </div>
              <div>
                Date: &nbsp;<span className="mil-dark">April 2023</span>
              </div>
              <div>
                Author: &nbsp;<span className="mil-dark">Paul Trueman</span>
              </div>
            </div>
            <div className="mil-p-120-120">
              <div className="swiper-container mil-2-slider mil-up">
                <div className="swiper-wrapper">
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-vertical mil-drag">
                      <img src="img/works/6/2.gif" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/6/2.gif"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-vertical mil-drag">
                      <img src="img/works/6/3.jpg" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/6/3.jpg"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-vertical mil-drag">
                      <img src="img/works/6/4.gif" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/6/4.gif"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                  <div className="swiper-slide">
                    <div className="mil-image-frame mil-vertical mil-drag">
                      <img src="img/works/6/5.jpg" alt="image" />
                      <a
                        data-fancybox="gallery"
                        data-no-swup=""
                        href="img/works/6/5.jpg"
                        className="mil-zoom-btn"
                      >
                        <img src="img/icons/zoom.svg" alt="zoom" />
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div className="row justify-content-between mil-p-120-0">
                <div className="col-lg-5">
                  <h3 className="mil-up mil-mb-60">
                    Simplicity, elegance, innovation!
                  </h3>
                </div>
                <div className="col-lg-6">
                  <p className="mil-up mil-mb-30">
                    A home surveillance camera that pays great attention to
                    security and user privacy, featuring two modes to provide
                    security while protecting personal privacy.The camera has an
                    open and closed mode, we define the product to have clear
                    two sides, expressing two working states and emotions.
                  </p>
                  <p className="mil-up">
                    Presents a simple and quiet state when not in use,
                    delivering a gentle and security.At the same time, the
                    camera can adapt to a variety of environments, providing
                    elegant ways of wall hanging and standing installation.
                  </p>
                </div>
              </div>
            </div>
            <div className="swiper-container mil-2-slider mil-up mil-mb-120">
              <div className="swiper-wrapper">
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-horizontal mil-drag">
                    <img src="img/works/6/6.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/6/6.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-horizontal mil-drag">
                    <img src="img/works/6/7.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/6/7.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
                <div className="swiper-slide">
                  <div className="mil-image-frame mil-horizontal mil-drag">
                    <img src="img/works/6/8.jpg" alt="image" />
                    <a
                      data-fancybox="gallery"
                      data-no-swup=""
                      href="img/works/6/8.jpg"
                      className="mil-zoom-btn"
                    >
                      <img src="img/icons/zoom.svg" alt="zoom" />
                    </a>
                  </div>
                </div>
              </div>
            </div>
            <div className="mil-works-nav mil-up">
              <a
                href="project-5.html"
                className="mil-link mil-dark mil-arrow-place mil-icon-left"
              >
                <span>Prev project</span>
              </a>
              <a href="/portfolio" className="mil-link mil-dark">
                <span>All projects</span>
              </a>
              <a
                href="#."
                className="mil-link mil-dark mil-arrow-place mil-disabled"
              >
                <span>Next project</span>
              </a>
            </div>
          </div>
        </section>
        {/* project end */}
        {/* call to action */}
        <section className="mil-soft-bg">
          <div className="container mil-p-120-120">
            <div className="row">
              <div className="col-lg-10">
                <span className="mil-suptitle mil-suptitle-right mil-suptitle-dark mil-up">
                  Looking to make your mark? We&apos;ll help you turn <br />
                  your project into a success story.
                </span>
              </div>
            </div>
            <div className="mil-center">
              <h2 className="mil-up mil-mb-60">
                Got a project <span className="mil-thin">in mind?</span>
                <br />
                Let&apos;s make it a reality
                <span className="mil-thin">together</span>
              </h2>
              <div className="mil-up">
                <a href="contact.html" className="mil-button mil-arrow-place">
                  <span>Contact us</span>
                </a>
              </div>
            </div>
          </div>
        </section>
        <div className="mil-hidden-elements">
          <div className="mil-dodecahedron">
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
            <div className="mil-pentagon">
              <div />
              <div />
              <div />
              <div />
              <div />
            </div>
          </div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            className="mil-arrow"
          >
            <path d="M 14 5.3417969 C 13.744125 5.3417969 13.487969 5.4412187 13.292969 5.6367188 L 13.207031 5.7226562 C 12.816031 6.1136563 12.816031 6.7467188 13.207031 7.1367188 L 17.070312 11 L 4 11 C 3.448 11 3 11.448 3 12 C 3 12.552 3.448 13 4 13 L 17.070312 13 L 13.207031 16.863281 C 12.816031 17.254281 12.816031 17.887344 13.207031 18.277344 L 13.292969 18.363281 C 13.683969 18.754281 14.317031 18.754281 14.707031 18.363281 L 20.363281 12.707031 C 20.754281 12.316031 20.754281 11.682969 20.363281 11.292969 L 14.707031 5.6367188 C 14.511531 5.4412187 14.255875 5.3417969 14 5.3417969 z" />
          </svg>
          <svg
            width={250}
            viewBox="0 0 300 1404"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="mil-lines"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M1 892L1 941H299V892C299 809.71 232.29 743 150 743C67.7096 743 1 809.71 1 892ZM0 942H300V892C300 809.157 232.843 742 150 742C67.1573 742 0 809.157 0 892L0 942Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 146V97L1 97V146C1 228.29 67.7096 295 150 295C232.29 295 299 228.29 299 146ZM300 96L0 96V146C0 228.843 67.1573 296 150 296C232.843 296 300 228.843 300 146V96Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 1H1V1403H299V1ZM0 0V1404H300V0H0Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 -4.37115e-08L150 1404L149 1404L149 0L150 -4.37115e-08Z"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 1324C232.29 1324 299 1257.29 299 1175C299 1092.71 232.29 1026 150 1026C67.7096 1026 1 1092.71 1 1175C1 1257.29 67.7096 1324 150 1324ZM150 1325C232.843 1325 300 1257.84 300 1175C300 1092.16 232.843 1025 150 1025C67.1573 1025 0 1092.16 0 1175C0 1257.84 67.1573 1325 150 1325Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M300 1175H0V1174H300V1175Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M150 678C232.29 678 299 611.29 299 529C299 446.71 232.29 380 150 380C67.7096 380 1 446.71 1 529C1 611.29 67.7096 678 150 678ZM150 679C232.843 679 300 611.843 300 529C300 446.157 232.843 379 150 379C67.1573 379 0 446.157 0 529C0 611.843 67.1573 679 150 679Z"
              className="mil-move"
            />
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M299 380H1V678H299V380ZM0 379V679H300V379H0Z"
              className="mil-move"
            />
          </svg>
        </div>
        {/* hidden elements end */}
      </div>
    </div>
  );
}
